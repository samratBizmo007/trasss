...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr style="background:#FCFBF3;">
		...
		<td style="padding:4px;">
			<a href='<?php echo site_url().'message/pm/message/'.$messages[$i][TF_PM_ID]; ?>'><?php echo $messages[$i][TF_PM_SUBJECT] ?></a>
		</td>
		<td style="padding:4px;">
	    ...
	    <?php if($type != MSG_SENT): ?>
		<td style="padding:4px;">
			<?php echo '<a href="'.site_url().'message/pm/send/'.$messages[$i][TF_PM_AUTHOR].'/RE&#58;'.$messages[$i][TF_PM_SUBJECT].'"> reply </a>' ?>
		</td>
		<?php endif; ?>
		<td style="padding:4px;" align="center">
			<?php if($type != MSG_DELETED)
					echo '<a href="'.site_url().'message/pm/delete/'.$messages[$i][TF_PM_ID].'/'.$type.'"> x </a>';
				  else
					echo '<a href="'.site_url().'message/pm/restore/'.$messages[$i][TF_PM_ID].'"> o </a>'; ?>
		</td>
	</tr>
	<?php endfor;?>
</table>
